...
# Clary CLI Installation Script
# Usage: curl -fsSL https://platform.clary.so/cli | bash

REPO="claryai/clary-cli"
BINARY_NAME="clary-cli"
INSTALL_NAME="clary"
VERSION="latest"
...
install_unix() {
    ...
    # Note: The archive contains 'clary' directly, not 'clary-cli'
...
install_windows() {
    ...
    # Move binary to install directory
    # Note: The archive contains 'clary.exe' directly, not 'clary-cli.exe'
    local extract_dir="$(dirname "$archive_path")"
...
main() {
    echo
    print_status "🚀 Clary CLI Installation Script"
    print_status "This script will download and install the Clary CLI binary"
    echo
    ...
    print_status "You can now use the 'clary' command to interact with the Clary CLI"
    print_status "Try running: clary --help"
    echo
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --beta)
            ...
            ;;
        --help)
            echo "Clary CLI Installation Script"
            echo
